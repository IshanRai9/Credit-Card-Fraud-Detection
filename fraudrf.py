...
# Function to train the model (Random Forest)
...
class FraudDetectionApp:
    ...
    def show_fraud_transactions(self):
        if self.y_pred is not None:
            fraud_count = np.sum(self.y_pred == 1)  # Count predicted fraud transactions
            actual_fraud_count = np.sum(self.y_test == 1)  # Count actual fraud transactions
            messagebox.showinfo("Fraud Transactions",
                                f"Number of Predicted Fraudulent Transactions: {fraud_count}\nNumber of Actual Fraudulent Transactions: {actual_fraud_count}")
